...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item active" aria-current="page">Exibir recurso</li>
@endsection
@section('styles-new')
    @parent
    ...
    <script>
        $(document).ready(function (){
            ...
            function change_title_page(estado) {
                if (estado === false) {
                    $("#pauta_titulo").parent().find('h1').html("EXIBIR RECURSO - Propinas Liquidadas");
                }
                else {
                    $("#pauta_titulo").parent().find('h1').html("EXIBIR RECURSO - Propinas Pedentes");
                }
            }
            ...
            function getStudentNotasPautaFinal() {  
                ...
                $.ajax({
                    url: "/avaliations/getStudentGradesRecurso/"+id_anoLectivo.val()+"/"+id_curso[0]+"/"+Turma_id_Select.val()+"/"+id_disciplinaVetor[0]+"/"+1,
                    type: "GET",
                    data: {
                        _token: '{{ csrf_token() }}'
                    },
                ...
                {  
                    ...
                    if (Object.keys(data).length > 0) {
                        ...
                        if(data['data']['estado_pauta']==1 && data['data']['estado_tipo'] == 10) {                          
                            ...
                            tabelatr+="<th  colspan='2'>CLASSIFICAÇÃO RECURSO</th>"
                            ...
                            $.each(resultados_student, function (index, item) { 
                                ...
                                if (selector_pauta === false) { 
                                    ...
                                    $.each(item, function (index_recurso, item_recurso) {
                                        ...
                                        console.log(item_recurso)

                                        if (item_recurso.estado_do_mes === "total") {
                                            if (aluno_nome != index) {
                                                aluno_nome = index;
                                                // console.log(index, index_recurso, item_recurso)
                                                numero_alunos_n += 1;
                            
                                                erro = 1;

                                                tabelatr+="<tr><td style='text-align: center'>"+numero_alunos_n+"</td>"
                                                tabelatr+="<<td style='text-align: center'>"+item_recurso.code_matricula+"</td>"
                                                ...
                                                if (item_recurso.MT_CodeDV === "Recurso") {
                                                ...
                                                if (Math.round(exame_pauta) > 9.5) {
                                                    //Mostra a nota do NEEN
                                                    if (neen == -1) {                                          
                                                        tabelatr+="<td style='text-align: center'> F </td>"
                                                        tabelatr+="<td class='c_final' style='text-align: left'>"+'Reprovado(a)'+"</td>"
                                                    }
                                                    else {
                                                        tabelatr+="<td style='text-align: center'>"+exame_pauta+"</td>"
                                                        tabelatr+="<td class='c_final' style='text-align: left'>"+'Aprovado(a)'+"</td>"
                                                    }
                                                } 
                                                else  {
                                                    //Mostra a nota do NEEN
                                                ...
                                                if (Math.round(calculo_mac) >= 10){
                                                    lista_alunos_notas.push([item_recurso.id_mat, item_recurso.user_id, exame_pauta, item_recurso.sexo, "@"])
                                                    alunos_aprovados += 1;
                                    ...
                                    $.each(item, function (index_recurso, item_recurso) {
                                        ...
                                        if (item_recurso.estado_do_mes === "pending") {
                                            if (aluno_nome != index) {
                                                aluno_nome = index;
                                                // console.log(index, index_recurso, item_recurso)
                                                numero_alunos_n += 1;
                                                ...
                                                if (Math.round(exame_pauta) > 9.5) {
                                                    ...
                                                    //Mostra a nota do NEEN
                                                    if (neen == -1) {                                          
                                                        tabelatr+="<td style='text-align: right'> - </td>"
                                                    }
                                                    else {
                                                        tabelatr+="<td style='text-align: right'>"+exame_pauta+"</td>"
                                                    }
                            ...
                            if (mensagem_erro_propina === 0) {
                                ...
                                $("#listaMenu").append("<center>Infelizmente não há estudantes com o emolumento pago, por favor verifica o seletor de escolha.</center>");
                            } 
                            if (mensagem_erro_propina === 2 || erro === 0) {
                                $("#listaMenu").empty();
                                
                                $("#listaMenu").append("<center>Felizmente não há estudantes com o emolumento não pago, por favor verifica o seletor de escolha para visualizar os estudantes com pagamento realizados.</center>");
                            } 
                            ...
                            estatistica_pauta.push([numero_alunos_n, alunos_aprovados, alunos_recurso, alunos_femenino, alunos_masculino, alunos_femenino_reprovado, alunos_masculino_reprovado]);
                            ...
                            $("#listaMenu").empty();
                            $("#lista_tr").empty();
                            tabelatr+="<td class='text-center'>A pauta não foi publicada, em caso de duvida contactar um superior.</td>"
                            $("#listaMenu").append(tabelatr); 
                        ...
                        $("#lista_tr").empty();
                        $("#listaMenu").empty();
                        tabelatr+="<td class='text-center'>Não exitem alunos ao exame nesta disciplina, em caso de duvida contactar um superior.</td>"
                        $("#listaMenu").append(tabelatr);
                    } 
