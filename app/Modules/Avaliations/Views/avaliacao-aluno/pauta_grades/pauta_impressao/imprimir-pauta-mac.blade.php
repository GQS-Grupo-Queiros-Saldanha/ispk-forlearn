...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item active" aria-current="page">Exibir MAC</li>
@endsection
@section('styles-new')
    @parent
...
@section('body')
    ...
    <div id="div_btn_save" class="p-2" style="right: 1rem;">
    ...
    <script>
        $(document).ready(function() {
            ...
            function change_title_page(estado) {
                if (estado === false) {
                    $("#pauta_titulo").parent().find('h1').html("Exibir MAC - Propinas Liquidadas");
                } else {
                    $("#pauta_titulo").parent().find('h1').html("Exibir MAC - Propinas Pedentes");
                }
            ...
            function getStudentNotasPautaFinal() {
                ...
                $.ajax({
                    url: "/avaliations/getStudentNotasPautaFinal/" + id_anoLectivo.val() + "/" + id_curso[
                            0] + "/" + Turma_id_Select.val() + "/" + id_disciplinaVetor[0] + "/" + 40 +
                        "/" + 0,
                    type: "GET",
                    data: {
                        _token: '{{ csrf_token() }}'
                    ...
                    if (data['data']['estado_pauta'] == 1 && data['data']['estado_tipo'] == 40) {
                        ...
                        $.each(resultados_student, function(index, item) {
                                    ...
                                    if (item_exam.estado_do_mes === "total") {
                                        ...
                                        if (aluno_nome != index) {
                                            ...
                                            $.each(item, function(index_avaliacao,item_avaliacao) {
                                                ...
                                                if (item_avaliacao.MT_CodeDV ===
                                                    "OA") {
                                                    if (oa_nome != item_avaliacao
                                                        .MT_CodeDV) {
                                                        if (item_avaliacao
                                                            .nota_anluno === null) {
                                                            tabelatr += "<td style='text-align: center'>" +'F' + "</td>"
                                            ...
                                            if (j >= 2) {
                                                ...
                                                // exame_pauta = ((parseFloat(calculo_mac) * mac_percentagem) / mac_percentagem);
                                                ...
                                                if (calculo_mac >= data['data']['avaliacao_config'].mac_nota_dispensa) {
                                                    ...
                                                    // lista_alunos_notas.push([item_exam.id_mat, item_exam.user_id, Math.round(exame_pauta)])
                                                    // alunos_reprovados += 1;
                                                    if (calculo_mac >= data['data']['avaliacao_config'].exame_nota_inicial && calculo_mac <= data['data']['avaliacao_config'].exame_nota_final) {
                            ...
                            // MOSTRA TODOS ESTUDANTES NA PAUTA
                            else {
                                if (a === 0) {
                                ...
                                $.each(item, function(index_exam, item_exam) {
                                    ...
                                    if (item_exam.estado_do_mes === "pending") {
                                        ...
                                        if (aluno_nome != index) {
                                            ...
                                            $.each(item, function(index_avaliacao,
                                                item_avaliacao) {
                                                // Estrura de repetição que pega as metricas PF1.
                                            ...
                                            if (j >= 2) {
                                                calculo_mac = macCalculate(data['data']['avaliacao_config'],pf1,pf1_percentagem,pf2,pf2_percentagem,oa,oa_percentagem);
                                                tabelatr +=
                                                    "<td style='text-align: right'>" + calculo_mac + " </td>"
                                                ...
                                                if (data['data']['exame'].has_mandatory_exam == 1) {
                                                    if (calculo_mac >= data['data']['avaliacao_config'].mac_nota_dispensa) {
                                                        ...
                                                        if(calculo_mac >= data['data']['avaliacao_config'].exame_nota_inicial && calculo_mac < data['data']['avaliacao_config'].exame_oral_final){
                                                            tabelatr+="<td class='c_final' style='text-align: center'>"+'Exame'+"</td>"
                                                            alunos_oral++;
                                                        }else{
                                                            if (calculo_mac >= data['data']['avaliacao_config'].exame_nota_inicial && calculo_mac <= data['data']['avaliacao_config'].exame_nota_final) {
