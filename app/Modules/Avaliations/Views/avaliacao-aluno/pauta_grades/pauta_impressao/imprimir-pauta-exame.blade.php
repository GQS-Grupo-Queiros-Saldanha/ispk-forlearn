...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item active" aria-current="page">Exibir exame escrito</li>
@endsection
@section('styles-new')
    @parent
    ...
    <script>
        $(document).ready(function() {
            ...
            function change_title_page(estado) {
                if (estado === false) {
                    $("#pauta_titulo").parent().find('h1').html("EXIBIR EXAME - Propinas Liquidadas");
                } else {
                    $("#pauta_titulo").parent().find('h1').html("EXIBIR EXAME - Propinas Pedentes");
                }
            }
            ...
            function getStudentNotasPautaFinal() {
                ...
                $.ajax({
                    // url: "/avaliations/getStudentMatriculation/" + id_anoLectivo.val() + "/" + id_curso[0] +
                    url: "/avaliations/getStudentMatriculation/" + id_anoLectivo.val() + "/" + id_curso[0] +
                        "/" + Turma_id_Select.val() + "/" + id_disciplinaVetor[0] + "/" +4+"/"+20,
                    type: "GET",
                    data: {
                        _token: '{{ csrf_token() }}'
                    },
                    ...
                    if (data['data']['estado_pauta'] == 1 && data['data']['estado_tipo'] == 20) {
                        ...
                        tabelatr += "<th colspan='2'>CLASSIFICAÇÃO EXAME EM ATRASO</th>"
                        ...
                        $.each(resultados_student, function(index, item) {
                            ...
                            // propina_nome = item[numero_alunos].article_name;
                            // mes_prpina = item[numero_alunos].mes > 9 ?item[numero_alunos].mes: "0"+item[numero_alunos].mes;
                                            ...
                                            if (exame_pauta >= data['data']['avaliacao_config'].exame_nota) {
                                                ...
                                                if (neen == -1) {
                                                    ...
                                                    if (neen == -1) {
                                                        ...
                                                        tabelatr += "<td style='text-align: right'>" + exame_pauta + "</td>"
                                                        if(exame_pauta>=data['data']['avaliacao_config'].exame_nota_inicial &&  exame_pauta<=data['data']['avaliacao_config'].exame_oral_final){
                                                        result="Exame Oral";
                                                        }
                                                        
                                                    }
                                                    tabelatr +="<td class='c_final' style='text-align: center'>" + result+ "</td>"
                                            }

                                            // LISTA DE ALUNOS E SUA MÉDIAS PARA SEREM TRANÇADOS
                                            if (calculo_mac >= data['data']['avaliacao_config'].exame_nota) {
                                                lista_alunos_notas.push([item_exam.id_mat,
                                                    item_exam.user_id, exame_pauta,
                                                    item_exam.sexo, "@"
                                                ])
                                                alunos_aprovados += 1;
