...
@foreach ($dados as $item)
    @if ($item->id_turma == $allDiscipline->id_turma && $item->id_disciplina == $allDiscipline->id_disciplina_no_plano)
        @if ($item->pauta_tipo == 'Pauta Seminário')
            @php
...
@if ($total_final == 1)
    @if (auth()->user()->hasAnyRole(['teacher', 'superadmin', 'coordenador-curso', 'Chefe_do_gabinete_de_termos']))
        @if (
            $prof == 'existe' ||
                $coordenador == 'existe' ||
                auth()->user()->hasAnyRole(['superadmin', 'Chefe_do_gabinete_de_termos']))
            <a tabindex="0" data-bs-toggle="tooltip" data-html="true"
                ...
                target="_blank" href="{{ route('viewFile.pautas_seminario', ['filename' => $link]) }}" class="btn "
