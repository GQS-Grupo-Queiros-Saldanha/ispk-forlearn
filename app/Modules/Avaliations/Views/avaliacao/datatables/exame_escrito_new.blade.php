...
@foreach ($dados_mac_1c as $item)
    @if ($item->id_turma == $allDiscipline->id_turma && $item->id_disciplina == $allDiscipline->id_disciplina_no_plano)
        @if ($item->pauta_tipo == 'Neen')
            @php
                $total_final = 1;
                $discipline_id = $item->id_disciplina;
                $metrica_id = $item->mt_id;
                $id_planoEstudo = $item->spe_id;
                $avaliacao_id = $item->avl_id;
                $turma = $item->id_turma;
                $lective_year_select = $item->lective_year;
                //$link = str_replace('/home/ispkforlearn', '', $item->path);
                //emarq
                $link = link_storage($item->path);
    ...
    @if (auth()->user()->hasAnyRole(['teacher', 'superadmin', 'coordenador-curso', 'Chefe_do_gabinete_de_termos', 'staff_matriculas']))
        @if (
            ...
            <a tabindex="0" data-bs-toggle="tooltip" data-html="true"
                ...
                target="_blank" href="{{ route('viewFile.pautas_mac', ['filename' => $link]) }}" class="btn "
