...
class MenuItemsController extends Controller
{
    ...
    public function index()
    {
        try {
            ...
            $menus = $menus->pluck('currentTranslation.display_name', 'id');

            $data = [
                'menus' => $menus
            ];

            return view('Cms::menu-items.index')->with($data);
    ...
    public function ajax(Request $request)
    {
        if ($request->has('id')) {
            ...
            // Ignorar a cache
            return app('model-cache')->runDisabled(function () use ($request) {
                
                return MenuItem::tree($request->get('id'));
    ...
    public function create()
    {
        ...
        return view('Cms::menu-items.menu-item')->with($data);
    }
    ...
    public function store(Request $request)
    {
        try {
            ...
            if (!empty($menu_item_translations)) {
                MenuItemTranslation::insert($menu_item_translations);
            }

            // Create permissions
            if ($request->has('permissions')) {
                $menu_item->syncPermissions($request->get('permissions'));
            }

            DB::commit();

            // Success message
            Toastr::success(__('Cms::menu-items.store_success_message'), __('toastr.success'));
    ...
    private function fetch($id, $action)
    {
        try {
            // Find
            $menu_item = MenuItem::whereId($id)->with([
            ...
            $data = [
                ...
                'permissions' => $permissions,
                //'icons' => $this->icons,
                'translations' => $menu_item->translations->keyBy('language_id')->toArray(),
                'languages' => Language::whereActive(true)->get()
            ];
            return view('Cms::menu-items.menu-item')->with($data);
        } catch (ModelNotFoundException $e) {
            Toastr::error(__('Cms::menu-items.not_found_message'), __('toastr.error'));
            Log::error($e);
    ...
    public function update(Request $request, $id)
    {
        try {
            ...
            // Find and update
            $menu_item = MenuItem::whereId($id)->firstOrFail();
            $menu_item->code = $request->get('code');
            $menu_item->external_link = $request->get('external_link');
            //$menu_item->icon = $request->get('icon');
            $menu_item->menus_id = $request->get('menus_id');
            $menu_item->parent_id = $request->has('parent_id') ? $request->get('parent_id') : null;
            $menu_item->save();

            // Disable previous translations
            MenuItemTranslation::where('menu_items_id', $menu_item->id)->update(['active' => false]);

            $version = MenuItemTranslation::where('menu_items_id', $menu_item->id)->whereLanguageId($default_language->id)->count() + 1;
            ...
            if ($request->has('permissions')) {
                ...
                $menu_item->syncPermissions([]);
            }

            DB::commit();

            // Success message
            Toastr::success(__('Cms::menu-items.update_success_message'), __('toastr.success'));
            return redirect()->route('menu-items.show', $id);
        } catch (ModelNotFoundException $e) {
            Toastr::error(__('Cms::menu-items.not_found_message'), __('toastr.error'));
            Log::error($e);
            return redirect()->back() ?? abort(500);
    ...
    public function destroy($id)
    {
        try {
            ...
            // Success message
            Toastr::success(__('Cms::menu-items.destroy_success_message'), __('toastr.success'));
            return redirect()->route('menu-items.index');
        ...
        try {
            if ($request->has('items')) {
                foreach ($request->get('items') as $item) {
                    $menu_item = MenuItem::whereId($item['id'])->firstOrFail();
                    $menu_item->parent_id = $item['parentId'] ?? null;
                    $menu_item->position = $item['order'];
                    $menu_item->save();
                }
            }
        } catch (Exception | Throwable $e) {
