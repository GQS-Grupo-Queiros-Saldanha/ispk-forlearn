        ...
        <table class="table  tabela_pauta table-striped table-hover tabela_pauta" id="{{ 'tabela_pauta_student' . $semestreActual }}">
            ...
            <thead>
                    <tr>
                        ...
                        <td class="rec bo1 text-center p-top" colspan="6">EXAME</td>
                    ...
                    <tr style="text-align: center">
                        ...
                        <th class="rec bo1" colspan="2">EXTRAORDINÁRIO</th> <!-- NOVA COLUNA -->
            ...
            <tbody>
                        ...
                        foreach ($percurso[$index] as $indexNotas => $itemNotas){
                            ...
                            if ($itemNotas->MT_CodeDV == 'oral') {
                                if ($itemNotas->nota_anluno == null) {
                                    ...
                                    $oral_nota = floatval($itemNotas->nota_anluno);
                                }
                            }
                            ...
                            // NOVO: Processar nota extraordinária
                            if ($itemNotas->MT_CodeDV == 'Extraordinário') {
                                if ($itemNotas->nota_anluno == null) {
                                    $extraordinario_nota = 0;
                                } else {
                                    $extraordinario_nota = floatval($itemNotas->nota_anluno);
                                }
                            }
                        }
                    }
                ...
                <!-- EXTRAORDINÁRIO -->
                @if ($extraordinario_nota == null)
                    <td style='text-align: center'> - </td>
                    <td style='text-align: center'> - </td>
                @else
                    @php
                        $p_extraordinario = mainController::verificar_pauta(
                            $id_turma,
                            $item_DISC->id_disciplina,
                            $item_DISC->id_anoLectivo,
                            'Pauta Extraordinário',
                        );
                    @endphp

                    @if ($p_extraordinario > 0)
                        @if ($estado_final == 'Aprovado(a)') 
                            <td style='text-align: center'>-</td>
                            <td style='text-align: center'>-</td>
                        @else
                            @php
                                $classificacao = round($extraordinario_nota);
                            ...
                            <td style='text-align: center'>{{ $extraordinario_nota }}</td>
                            <td class="{{'text-bold text-center ' . $color_final}}">{{ $estado_final }}</td>
                        @endif
                    @else
                        <td style='text-align: center'>-</td>
