...
@if (isset($articles['dividas']['pending']) && $articles['dividas']['pending'] > 0 && request()->ip() != '129.122.187.217')
            ...
            @foreach ($disciplines as $index => $item_DISC)
                    ...
                    @php 
                        ...
                        $code_disc = false;
                                ...
                                // 1️⃣ Tentativa: método antigo
                                $turma = $classes->first(function ($item) use ($item_DISC) {
                                    return isset($item->display_name[3], $item_DISC->code_disciplina[3]) &&
                                        $item->display_name[3] === $item_DISC->code_disciplina[3];
                                });

                                // 2️⃣ Fallback: método alternativo
                                if (!$turma) {
                                    $turma = $classes->first(function ($item) use ($item_DISC) {
                                        return isset($item_DISC->code_disciplina[3]) &&
                                            (string) $item->year === substr($item_DISC->code_disciplina, 3, 1);
                                    });
                                }
