    ...
    <main>
        ...
        @php
                   ...
                   <b style="">
                    ...
                    {{ \Carbon\Carbon::parse($studentInfo->emitido)->format('d/m/Y') }}, estudante do {{ $studentInfo->year ?? 'Ano curricular' }}º Ano,
                    no curso de {{ $studentInfo->course ?? '(nome do curso)' }}, matriculado(a) com o nº {{ $studentInfo->number }},
                     solicitou o pedido de transferência 
                    (de saída do {{$institution->abrev}}),  destinado a/ao {{ $transference->school_name ?? '(nome da instituição)' }}.
            ...
            <p> 
                    ...
                    Passa-se a presente declaração nº {{$requerimento->code ?? 'código doc'}}, liquidada no Comprovativo de pagamento nº {{$recibo ?? 'recibo'}},
