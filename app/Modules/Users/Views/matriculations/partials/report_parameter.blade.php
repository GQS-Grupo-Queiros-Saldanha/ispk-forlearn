...
<div @if(isset($child) && $child) data-parameter="{{ $parameter->id }}" @endif class="@if(isset($child) && $child) p-1 collapse bg-light-grey @endif">
    @php
    ...
    @switch($parameter->type)
        ...
        <div class="form-group">
            ...
            <select @if($parameter->options->contains('has_related_parameters', true)) data-options-have-related-parameters="true" @endif name="{{ $name }}" class="form-control form-control-sm{{ $errors->has($name) ? ' is-invalid' : '' }}" readonly disabled style="display: none">
                ...
                @foreach($options_with_related_parameters as $option)
                    @foreach($option->relatedParametersRecursive as $parameter)
                        @if(!in_array($parameter->type, ['file_doc', 'file_pdf'], true))
                            @include('Users::users.partials.pdf_parameter', ['parameter' => $parameter, 'action' => $action, 'parameter_group' => $parameter_group, 'user' => $user, 'child' => true])
                        @endif
                    @endforeach
                @endforeach
            @endif
