...
@switch($action)
    @case('create')
        @section('page-title', __('Users::matriculations.create_matriculation') . ' por equivalência - ' .
    ...
    @section('breadcrumb')
        ...
        <li class="breadcrumb-item">
            <a href="{{ route('matriculations.index') }}">Matrículas equivalência</a>
        </li>
            ...
            <div class="container-fluid">
                ...
                @switch($action)
                    @case('create')
                        ...
                        {!! Form::open(['route' => ['confirmation_matriculation_equivalence.store']]) !!}
                    @break
                                ...
                                <div class="col-6">
                                    <div class="form-group">
                                        @if ($action === 'create')
                                            ...
                                            <select name="" class="form-control" id="IdAnoCurricular">
                                                <option value="">Selecione o ano para matrícula</option>
            ...
            (() => {
                ...
                function switchUser(userId) {
                    ...
                    if (userId) {
                        ...
                        let route = ("{{ route('confirmations.equivalence.ajax', 'id_user') }}").replace('id_user',
                            IDUSER_LECTIVE_YEAR);
                ...
                @if ($action === 'create')
                    ...
                    if (!$.isEmptyObject(selectUser)) {
                        ...
                        selectUser.change(function() {
                            const curricularAno = $("#IdAnoCurricular").val("");
                            $('#discipline-tables-containerx').empty();
                        });

                        $("#IdAnoCurricular").change(function(e) {
                            const curricular = $("#IdAnoCurricular").val();
                            
                            if(selectUser.val() != "" && curricular != "") {
                                switchUser(selectUser.val());
                                
                                setTimeout(() => {
                                    hideDataDiscipline(curricular);
                ...
                function hideDataDiscipline(selectedValue) {
                                const container = $('#ContainerSeguinte');

                             elementos = $('#ContainerSeguinte').children();
                            var years =  [];

                            for (let i = 0; i < elementos.length; i++) {
                                string = $(elementos[i]).attr('class');
                                ...
                                const totalCount = $('#ContainerSeguinte').attr('data-count'); 
                                for (let i = 0; i < years.length; i++) {
                                    const div = $(`.only_grade_${years[i]}`);
                                    if (years[i] > selectedValue) {
                                        div.remove();  
                                    } 
                                }
                  }
