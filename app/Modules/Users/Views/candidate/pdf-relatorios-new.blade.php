  ...
  $vaga_manha_f_global  
    ...
    if ($key == 'total') {
        ...
        if (is_array($item)) {
            ...
            // Se não for array, assume valores padrões (ou calcula com base no próprio $item)
            $total_valor = is_numeric($item) ? $item : 0;
            $total = is_numeric($item) ? $item : 0;
            $value = 0;
        }

        $liquidada += $total_valor;
        $liquidada_t = $total;

        $value_base = $value != 0 ? $value : 0;
