@section('title',__('RH - criar permissão'))
                ...
                <div class="associarCodigo">
                    <div class="ml-0 mr-0 pl-0 pr-0  pb-4 row col-12 ">
                        ...
                        <h5 class="col-md-12 mb-3 text-right text-muted text-uppercase">
                            @switch($action)
                                @case('create') @lang('Users::permissions.create_permission') @break
                                @case('show') @lang('Users::permissions.permission') @break
                                @case('edit') @lang('Users::permissions.edit_permission') @break
                            @endswitch
                        </h5>
                                ...
                                <div class="col" style="background-color: #f5fcff">
                                    ...
                                    <div class="card">
                                        <div class="card-body" style="background-color: #f5fcff">
                                            {{ Form::bsText('name', null, ['placeholder' => __('Users::permissions.name'), 'disabled' => $action === 'show', 'required']) }}
                                        </div>
                                    </div>
                                            ...
                                            <div class="tab-content">
                                                @foreach($languages as $language)
                                                    <div class="tab-pane @if($language->default) active show @endif" id="language{{ $language->id }}">
                                                        {{ Form::bsText('display_name['.$language->id.']', $action === 'create' ? old('display_name.'.$language->id) : $translations[$language->id]['display_name'], ['placeholder' => __('translations.display_name'), 'disabled' => $action === 'show', !$language->default ?: 'required'], ['label' => __('translations.display_name')]) }}
                                                        {{ Form::bsText('description['.$language->id.']', $action === 'create' ? old('description.'.$language->id) : $translations[$language->id]['description'], ['placeholder' => __('translations.description'), 'disabled' => $action === 'show', !$language->default ?: 'required'], ['label' => __('translations.description')]) }}
                                                    </div>
                                                @endforeach
                                            </div>
                                        </div>
