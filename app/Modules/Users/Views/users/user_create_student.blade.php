@switch($action)
@case('create') @section('title','ESTUDANTE| CRIAR') @break
@case('show') @section('title','ESTUDANTE') @break
@case('edit') @section('title','ESTUDANTE| EDITAR') @break
@endswitch
                ...
                <div class="row --mb-2">
                    <div class="col-sm-6">
                        <h1 class="m-0 text-dark">
                            @switch($action)
                                @case('create') CRIAR ESTUDANTE @break
                        ...
                        <div class="card">
                            ...
                            @if($action === 'create')
                                <div class="card-body row pb-0">
                                    {{ Form::bsText('full_name', null, ['placeholder' => __('Users::users.name'), 'disabled' => $action === 'show', 'required', 'autocomplete' => 'name'], ['label' => __('Users::users.name')]) }}
                        ...
                        <div class="float-right">
                            @switch($action)
                                ...
                                <div hidden id="nextBtn">
                                    @if(auth()->user()->hasAnyRole(['superadmin', 'staff_forlearn']) || auth()->user()->hasAnyPermission(['create_student']))
                                        <button type="submit" class="btn btn-success mb-3 mr-3">
                                            @icon('fas fa-plus-circle')
                                            @lang('common.create') estudante
                                        </button>
                                    @endif
                                </div>
                                @break
...
@section('scripts')
    ...
    <script>
        $(function () {
            ...
            if ($('input[name="parameters[3][14]"]') && $('input[name="parameters[3][14]"]').val().match(/^[0-9]{9}[A-Z]{2}[0-9]{3}$/)) {
                    var file = $('input[name="attachment_parameters[1][25]"]').val();
                    var extension = file.substr((file.lastIndexOf('.') +1));
