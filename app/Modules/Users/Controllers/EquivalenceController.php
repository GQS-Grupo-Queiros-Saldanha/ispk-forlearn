...
class EquivalenceController extends Controller
{
    ...
    public function ajaxTransfereStudent($lective){
     ...
     $model = DB::table('tb_transference_studant as transf')
       ...
       ->where('transf.lectiveYear', $lective)
   ...
   return Datatables::of($model)
       ->addColumn('actions', function ($item) {
           return view('Users::equivalence.datatables.actionEquivalence')->with('item',$item);
       })
       ->addColumn('in_out', function ($in_out) {
           if($in_out->type_transference==1)
           return "Entrada";
           if($in_out->type_transference==2)
           return "Saída";
           if($in_out->type_transference==3)
           return "Mudança de curso";
        
       })
       ->addColumn('states', function($state){
           return view('Users::equivalence.datatables.status_equivalence')->with('state',$state);
       })
...
public function transference_studant_store(Request $request){
    ...
    $data = explode(',', $request->get('tipe_transference'));
    
    $tipe_transference = $data[0];
    $doc_type = $data[1];
    
    
        //codev dos emolumentos
      $tipe_transference == 1 ? $article_id = "pedido_t_entrada" : $article_id ="pedido_t_saida";
     ...
     if($emolumento_transferencia->isEmpty()){
      ...
      if(!$consulta->isEmpty()){
        ...
        Toastr::warning(__('A forLEARN detectou que já existe uma marcação de pedido de transferência para este estudante, por favor verifique a tabela para validar a mesma , caso contrário contacte o apoio a forLEARN'), __('toastr.warning'));
        return redirect()->back();
        }
     ...
     $transference_request = DB::table('tb_transference_studant')->insertGetId([
        'user_id' => $request->students,
        'lectiveYear' => $request->anoLectivo,
        'school_name' =>$request->school_name,
        'type_transference' =>$tipe_transference,
     ...
     DB::table('requerimento')->insert(
                        [
                            ...
                            'codigo_documento' => $doc_type,
                            'transference_request' => $transference_request
                        
                        ]
                    ); 
      
      
       if(!($tipe_transference == 1)){
          
          $articles = DB::table('articles')
                            ->where('anoLectivo', $request->anoLectivo)
                            ->whereIn('id_code_dev', [26, 27,28])
                            ->select(['id', 'id_code_dev'])
                            ->get()
                            ->toArray();
                            
           
                            
                   // gerar plano curricular das disciplinas 
                   
                    $ar = array_filter($articles, function($article){
                            return $article->id_code_dev == 26;
            });
            $articleRequest = [];
            
            array_push($articleRequest, ...$ar);
           
            
            $articleRequest = $articleRequest[0];
            
            if(isset($articleRequest) && $articleRequest->id_code_dev == 26)
            {
                 $article = createAutomaticArticleRequest($request->students, $articleRequest->id, null, null);
             
              // GUARDAR REQUERIMENTO PLANO CURRICULAR DAS DISCIPLINAS
            ...
            // gerar declarações com notas
              $ar = array_filter($articles, function($article){
                            return $article->id_code_dev == 27;
            });
            $articleRequest = [];
            array_push($articleRequest, ...$ar);
            $articleRequest = $articleRequest[0];
            
            if(isset($articleRequest) && $articleRequest->id_code_dev == 27)
            {
                
        
                
                  $ultimo_ano_concluido =  DB::table('new_old_grades as notas')
                 ->where('notas.user_id',$request->students)
                 ->where('notas.grade', '>', 9)
                ->join('study_plans_has_disciplines as sphd',"sphd.disciplines_id","notas.discipline_id")
                ->orderBy('years', 'DESC')
                ->pluck('years')
                ->first();
     
               
        for($ano = 1; $ano <= $ultimo_ano_concluido; $ano++ ){
             ...
             $doc_type = 2;
              ...
              DB::table('requerimento')->insert(
                   ...
                   // gerar plano curricular do curso
                   
                    $ar = array_filter($articles, function($article){
                            return $article->id_code_dev == 28;
            });
            $articleRequest = [];
            array_push($articleRequest, ...$ar);
            $articleRequest = $articleRequest[0];
            
            if(isset($articleRequest) && $articleRequest->id_code_dev == 28)
            {
                 $article = createAutomaticArticleRequest($request->students, $articleRequest->id, null, null);
             
              // GUARDAR REQUERIMENTO PLANO CURRICULAR DO CURSO
    ...
    private function gerar_codigo_documento($requerimento_id){
        ...
        if(isset($last_code->code) && ($last_code->year == $requerimento->year)){
            $last_code->code = substr($last_code->code, 3);
            $code =  1 + intval($last_code->code);
        }else{
            $code = 1;
        }
        
            $code  = substr($requerimento->ano_lectivo, 0, 2) . '-' . sprintf('%04d', $code);
   ...
   
    {
        ...
        return view('Users::equivalence.chance-course.index')->with($data);
