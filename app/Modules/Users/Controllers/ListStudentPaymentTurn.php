...
class ListStudentPaymentTurn extends Controller
{
    ...
    public function index()
    {
        try {
            ...
            return view('Users::list-class-turn.index', compact('lectiveYears', 'lectiveYearSelected', 'courses'));
    ...
    public function ajaxClassDataPDF(Request $request)
    {
        ...
        try {
            ...
            $article_rule_valor = $model->map(function ($item) {
                // Busca o artigo da propina para o usuário atual
                $result = DB::table('article_requests as user_emolumento')
                    ->join('articles as article_emolumento', 'user_emolumento.article_id', '=', 'article_emolumento.id')
                    ->where('user_emolumento.user_id', $item->user_id)
                    ->where('article_emolumento.id_code_dev', DB::raw("(SELECT id FROM code_developer WHERE code = 'propina' LIMIT 1)"))
                    ->select('article_emolumento.id')
                    ->first(); // Retorna apenas um registro

                // Se não encontrar um artigo, retorna false para remover do filtro
                if (!$result) {
                    return '';
                }

                // Agora buscamos o valor do article_rules correspondente ao article encontrado
                $valor = DB::table('artcles_rules')
                    ->where('id_articles', 309)
                    ->select('artcles_rules.valor')
                    ->first();

                //dd($valor); // Debugando o valor encontrado

                return  $valor; // Mantém apenas registros válidos no filtro
            })->filter()->first();
            ...
            $pdf = PDF::loadView("Users::list-class-turn.pdf_lista", compact(
                'model',
                'regime',
                'turmaC',
                'curso',
