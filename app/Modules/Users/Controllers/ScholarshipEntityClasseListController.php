...
class ScholarshipEntityClasseListController extends Controller
{ 
    public function index()
    {
        ...
        $courses = Course::with([
            ...
            //return view('Users::scholarship_entity.index');
            return view('Users::scholarship_entity.index', compact('lectiveYears', 'lectiveYearSelected','courses'));
        }
  ...
  $model=DB::table('matriculation_classes as mat_class')
        ...
        ->join('scholarship_holder as hold','hold.user_id','mat.user_id')
        ->where('are_scholarship_holder', 1)
        ->join('scholarship_entity as ent', function ($join) use ($request) {
            $join->on('ent.id', 'hold.scholarship_entity_id')
                 ->where('ent.type', $request->status);
        })
      ...
      //fim dos pagos 
       ...
       $pdf = PDF::loadView("Users::scholarship_entity.pdf_lista", compact(
             'model',
             'regime',
             'turmaC',
             'curso',
