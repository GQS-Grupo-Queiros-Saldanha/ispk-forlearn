...
<main>
    <div class="row">
        <div class="col-12">
            <br>
            <center>
                @php $url = "https://" . $_SERVER['HTTP_HOST'] . "/storage/" . $institution->logotipo; @endphp
    ...
    <div class="row">
        <div class="col-md-12">
            @foreach ($model as $roleName => $users)
                @if (!$loop->first)
                    <div class="page-break"></div>
                @endif
                <table id="table-classes" class="article-table" style="margin-bottom: 50px;">
                    <thead>
                        <tr>
                            <th class="header-title" colspan="10">{{ $roleName }}</th> <!-- Nome do Curso -->
                        </tr>
                        <tr>
                    ...
                    <tbody>
                        @foreach ($users as $index => $user)
                            <tr>
                                ...
                                <td class="text-center"><b></b>{{ $user->matricula }}</td>
                                <td class="text-center"><b></b>{{ $user->n_bi }}</td>

                                <td class="text-center"><b></b>{{ $user->nome_completo }}</td>
                                <td class="text-center"><b></b>{{ $user->email }}</td>
