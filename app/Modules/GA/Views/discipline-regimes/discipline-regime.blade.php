@switch($action)
    @case('create') @section('title',__('GA::discipline-regimes.create_discipline_regime')) @break
@case('show') @section('title',__('GA::discipline-regimes.discipline_regime')) @break
@case('edit') @section('title',__('GA::discipline-regimes.edit_discipline_regime')) @break
@endswitch
    ...
    @include("GA::navbar.navbar")
                ...
                <div class="row mb-2">
                    <div class="col-sm-6">
                        <h1 class="m-0 text-dark">
                            @switch($action)
                                @case('create') @lang('GA::discipline-regimes.create_discipline_regime') @break
                                @case('show') @lang('GA::discipline-regimes.discipline_regime') @break
                                @case('edit') @lang('GA::discipline-regimes.edit_discipline_regime') @break
                            @endswitch
                        </h1>
                    </div>
                    <div class="col-sm-6">
                        @switch($action)
                            @case('create') {{ Breadcrumbs::render('discipline-regimes.create') }} @break
                            @case('show') {{ Breadcrumbs::render('discipline-regimes.show', $discipline_regime) }} @break
                            @case('edit') {{ Breadcrumbs::render('discipline-regimes.edit', $discipline_regime) }} @break
                        @endswitch
                    </div>
                </div>
            </div>
        ...
        <div class="content">
            <div class="container-fluid">
                ...
                @switch($action)
                    @case('create')
                    {!! Form::open(['route' => ['discipline-regimes.store']]) !!}
                    @break
                    @case('show')
                    {!! Form::model($discipline_regime) !!}
                    @break
                    @case('edit')
                    {!! Form::model($discipline_regime, ['route' => ['discipline-regimes.update', $discipline_regime->id], 'method' => 'put']) !!}
                    @break
                @endswitch
                ...
                <div class="row">
                    <div class="col">
                        ...
                        @switch($action)
                            ...
                            @break
                            @case('show')
                            <a href="{{ route('discipline-regimes.edit', $discipline_regime->id) }}" class="btn btn-sm btn-warning mb-3">
                                @icon('fas fa-edit')
                                @lang('common.edit')
                            </a>
                            @break
