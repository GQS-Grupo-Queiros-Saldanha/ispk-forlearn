...
<nav class="navbar navbar-expand-lg navbar-dark bg-dark " style="margin:0px!important;padding:0!important!">
    ...
    <a class="navbar-brand navbar-logo inicio-nav tirar" href="#"><i class="fas fa-bookmark"></i>BIBLIOTECA
    </a>
    ...
    <div class="collapse navbar-collapse dropdown ani-d" id="navbarSupportedContent">
        ...
        @if (auth()->user()->hasAnyPermission(['library_manage_item']) ||
            ...
            auth()->user()->hasAnyPermission(['library_manage_request']) ||
            auth()->user()->hasAnyPermission(['library_view'])
            )
            <ul class="navbar-nav mr-auto">
                ...
                <li class="nav-item dropdown li-nav">
                    ...
                    <a class="nav-link dropdown-toggle a-first" href="#" id="navbarDropdown" role="button"
                        ...
                        <i class="fas fas fa-handshake"></i>Requisições
                    </a>
                    <div class="dropdown-menu" aria-labelledby="navbarDropdown">
                        @if (auth()->user()->hasAnyPermission(['library_manage_item','library_manage_request']))
                        <a class="dropdown-item" href="{{ route('library-loan') }}"><i class="fa fa-book"></i>Livro</a>
                        <a class="dropdown-item" href="{{ route('library-computer-loan') }}"><i
                                class="fa fa-desktop"></i>Computador</a>
                        @endif
                        <a class="dropdown-item" href="{{ route('library-searchLoan') }}"><i
                                class="fas fas fa-handshake"></i>Gerir
                            requisição</a>
        ...
        @endif
        @if (auth()->user()->hasAnyPermission(['library_manage_item','library_view']) ||
            auth()->user()->hasRole(['superadmin']))
            <ul class="navbar-nav nav-opcao">
                <li class="nav-item dropdown li-nav">
                    ...
                    <div class="dropdown-menu" aria-labelledby="navbarDropdown">
                        @if (auth()->user()->hasAnyPermission(['library_manage_item','library_view']))
                            <a class="dropdown-item" href="{{ route('library-searchBooks') }}"><i
                                    class="fa fa-cogs"></i>Gerir
                                Itens</a>
                        @endif
                        {{-- <a class="dropdown-item" id="menu_time" href="#" style="display: none" 
                     data-target="#modalTempo" data-toggle="modal"><i class="fa fa-clock"></i>Tempo</a> --}}
                        @if (auth()->user()->hasAnyRole(['superadmin']))
                            <a class="dropdown-item" href="{{ route('library-bin') }}"><i
                                    class="fa fa-cogs"></i>Restaurar
                                Itens</a>
                        @endif
                    </div>
                </li>
