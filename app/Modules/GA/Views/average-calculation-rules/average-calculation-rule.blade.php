@switch($action)
    @case('create') @section('title',__('GA::average-calculation-rules.create_average_calculation_rule')) @break
@case('show') @section('title',__('GA::average-calculation-rules.average_calculation_rule')) @break
@case('edit') @section('title',__('GA::average-calculation-rules.edit_average_calculation_rule')) @break
@endswitch
    ...
    <div class="content-panel" style="padding: 0px">
                ...
                <div class="row mb-2">
                    <div class="col-sm-6">
                        <h1 class="m-0 text-dark">
                            @switch($action)
                                @case('create') @lang('GA::average-calculation-rules.create_average_calculation_rule') @break
                                @case('show') @lang('GA::average-calculation-rules.average_calculation_rule') @break
                                @case('edit') @lang('GA::average-calculation-rules.edit_average_calculation_rule') @break
                            @endswitch
                        </h1>
                    </div>
                    <div class="col-sm-6">
                        @switch($action)
                            @case('create') {{ Breadcrumbs::render('average-calculation-rules.create') }} @break
                            @case('show') {{ Breadcrumbs::render('average-calculation-rules.show', $average_calculation_rule) }} @break
                            @case('edit') {{ Breadcrumbs::render('average-calculation-rules.edit', $average_calculation_rule) }} @break
                        @endswitch
                    </div>
                </div>
            </div>
        ...
        <div class="content">
            <div class="container-fluid">
                ...
                @switch($action)
                    @case('create')
                    {!! Form::open(['route' => ['average-calculation-rules.store']]) !!}
                    @break
                    @case('show')
                    {!! Form::model($average_calculation_rule) !!}
                    @break
                    @case('edit')
                    {!! Form::model($average_calculation_rule, ['route' => ['average-calculation-rules.update', $average_calculation_rule->id], 'method' => 'put']) !!}
                    @break
                @endswitch
                ...
                <div class="row">
                    <div class="col">
                        ...
                        @switch($action)
                            ...
                            @break
                            @case('show')
                            <a href="{{ route('average-calculation-rules.edit', $average_calculation_rule->id) }}" class="btn btn-sm btn-warning mb-3">
                                @icon('fas fa-edit')
                                @lang('common.edit')
                            </a>
                            @break
