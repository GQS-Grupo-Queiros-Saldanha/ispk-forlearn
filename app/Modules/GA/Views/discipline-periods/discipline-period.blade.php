@switch($action)
    @case('create') @section('title',__('GA::discipline-periods.create_discipline_period')) @break
@case('show') @section('title',__('GA::discipline-periods.discipline_period')) @break
@case('edit') @section('title',__('GA::discipline-periods.edit_discipline_period')) @break
@endswitch
    ...
    @include("GA::navbar.navbar")
                ...
                <div class="row mb-2">
                    <div class="col-sm-6">
                        <h1 class="m-0 text-dark">
                            @switch($action)
                                @case('create') @lang('GA::discipline-periods.create_discipline_period') @break
                                @case('show') @lang('GA::discipline-periods.discipline_period') @break
                                @case('edit') @lang('GA::discipline-periods.edit_discipline_period') @break
                            @endswitch
                        </h1>
                    </div>
                    <div class="col-sm-6">
                        @switch($action)
                            @case('create') {{ Breadcrumbs::render('discipline-periods.create') }} @break
                            @case('show') {{ Breadcrumbs::render('discipline-periods.show', $discipline_period) }} @break
                            @case('edit') {{ Breadcrumbs::render('discipline-periods.edit', $discipline_period) }} @break
                        @endswitch
                    </div>
                </div>
            </div>
        ...
        <div class="content">
            <div class="container-fluid">
                ...
                @switch($action)
                    @case('create')
                    {!! Form::open(['route' => ['discipline-periods.store']]) !!}
                    @break
                    @case('show')
                    {!! Form::model($discipline_period) !!}
                    @break
                    @case('edit')
                    {!! Form::model($discipline_period, ['route' => ['discipline-periods.update', $discipline_period->id], 'method' => 'put']) !!}
                    @break
                @endswitch
                ...
                <div class="row">
                    <div class="col">
                        ...
                        @switch($action)
                            ...
                            @break
                            @case('show')
                            <a href="{{ route('discipline-periods.edit', $discipline_period->id) }}" class="btn btn-sm btn-warning mb-3">
                                @icon('fas fa-edit')
                                @lang('common.edit')
                            </a>
                            @break
