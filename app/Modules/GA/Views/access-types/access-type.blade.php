@switch($action)
    @case('create') @section('title',__('GA::access-types.create_access_type')) @break
@case('show') @section('title',__('GA::access-types.access_type')) @break
@case('edit') @section('title',__('GA::access-types.edit_access_type')) @break
@endswitch
    ...
    <div class="content-panel" style="padding: 0px">
                ...
                <div class="row mb-2">
                    <div class="col-sm-6">
                        <h1 class="m-0 text-dark">
                            @switch($action)
                                @case('create') @lang('GA::access-types.create_access_type') @break
                                @case('show') @lang('GA::access-types.access_type') @break
                                @case('edit') @lang('GA::access-types.edit_access_type') @break
                            @endswitch
                        </h1>
                    </div>
                    <div class="col-sm-6">
                        @switch($action)
                            @case('create') {{ Breadcrumbs::render('access-types.create') }} @break
                            @case('show') {{ Breadcrumbs::render('access-types.show', $access_type) }} @break
                            @case('edit') {{ Breadcrumbs::render('access-types.edit', $access_type) }} @break
                        @endswitch
                    </div>
                </div>
            </div>
        ...
        <div class="content">
            <div class="container-fluid">
                ...
                @switch($action)
                    @case('create')
                    {!! Form::open(['route' => ['access-types.store']]) !!}
                    @break
                    @case('show')
                    {!! Form::model($access_type) !!}
                    @break
                    @case('edit')
                    {!! Form::model($access_type, ['route' => ['access-types.update', $access_type->id], 'method' => 'put']) !!}
                    @break
                @endswitch
                ...
                <div class="row">
                    <div class="col">
                        ...
                        @switch($action)
                            ...
                            @break
                            @case('show')
                            <a href="{{ route('access-types.edit', $access_type->id) }}" class="btn btn-sm btn-warning mb-3">
                                @icon('fas fa-edit')
                                @lang('common.edit')
                            </a>
                            @break
