@switch($action)
    @case('create') @section('title',__('GA::discipline-profiles.create_discipline_profile')) @break
@case('show') @section('title',__('GA::discipline-profiles.discipline_profile')) @break
@case('edit') @section('title',__('GA::discipline-profiles.edit_discipline_profile')) @break
@endswitch
    ...
    @include("GA::navbar.navbar")
                ...
                <div class="row mb-2">
                    <div class="col-sm-6">
                        <h1 class="m-0 text-dark">
                            @switch($action)
                                @case('create') @lang('GA::discipline-profiles.create_discipline_profile') @break
                                @case('show') @lang('GA::discipline-profiles.discipline_profile') @break
                                @case('edit') @lang('GA::discipline-profiles.edit_discipline_profile') @break
                            @endswitch
                        </h1>
                    </div>
                    <div class="col-sm-6">
                        @switch($action)
                            @case('create') {{ Breadcrumbs::render('discipline-profiles.create') }} @break
                            @case('show') {{ Breadcrumbs::render('discipline-profiles.show', $discipline_profile) }} @break
                            @case('edit') {{ Breadcrumbs::render('discipline-profiles.edit', $discipline_profile) }} @break
                        @endswitch
                    </div>
                </div>
            </div>
        ...
        <div class="content">
            <div class="container-fluid">
                ...
                @switch($action)
                    @case('create')
                    {!! Form::open(['route' => ['discipline-profiles.store']]) !!}
                    @break
                    @case('show')
                    {!! Form::model($discipline_profile) !!}
                    @break
                    @case('edit')
                    {!! Form::model($discipline_profile, ['route' => ['discipline-profiles.update', $discipline_profile->id], 'method' => 'put']) !!}
                    @break
                @endswitch
                ...
                <div class="row">
                    <div class="col">
                        ...
                        @switch($action)
                            ...
                            @break
                            @case('show')
                            <a href="{{ route('discipline-profiles.edit', $discipline_profile->id) }}" class="btn btn-sm btn-warning mb-3">
                                @icon('fas fa-edit')
                                @lang('common.edit')
                            </a>
                            @break
