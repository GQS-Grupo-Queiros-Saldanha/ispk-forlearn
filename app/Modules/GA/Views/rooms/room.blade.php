@switch($action)
    @case('create') @section('title',__('GA::rooms.create_room')) @break
    @case('show') @section('title',__('GA::rooms.room')) @break
    @case('edit') @section('title',__('GA::rooms.edit_room')) @break
@endswitch
                ...
                <div class="row mb-2">
                    <div class="col-sm-6">
                        <h1 class="m-0 text-dark">
                            @switch($action)
                                @case('create') @lang('GA::rooms.create_room') @break
                                @case('show') @lang('GA::rooms.room') @break
                                @case('edit') @lang('GA::rooms.edit_room') @break
                            @endswitch
                        </h1>
                    </div>
                    <div class="col-sm-6">
                        @switch($action)
                            @case('create') {{ Breadcrumbs::render('rooms.create') }} @break
                            @case('show') {{ Breadcrumbs::render('rooms.show', $room) }} @break
                            @case('edit') {{ Breadcrumbs::render('rooms.edit', $room) }} @break
                        @endswitch
                    </div>
                </div>
            </div>
                ...
                <div class="row">
                    <div class="col">
                        ...
                        @switch($action)
                            ...
                            @break
                            @case('show')
                            <a href="{{ route('rooms.edit', $room->id) }}" class="btn btn-sm btn-warning mb-3">
                                @icon('fas fa-edit')
                                @lang('common.edit')
                            </a>
                            @break
                        ...
                        <div class="card">
                            <div class="card-body">
                                ...
                                @include('GA::rooms.partials.buildings')
                            ...
                            <div class="card-header d-flex p-0">
                                ...
                                <ul class="nav nav-pills ml-auto p-2">
                                    @foreach($languages as $language)
                                        <li class="nav-item">
                                            <a class="nav-link @if($language->default) active show @endif" href="#language{{ $language->id }}" data-toggle="tab">{{ $language->name }}</a>
                                        </li>
                                    @endforeach
                                </ul>
                            </div>
